...
def list_offers(
    ...
    **kwargs,
    ...
    Backward compatible: older main.py may pass show/invoice/paid/client/q.
    - show: 'active' (default), 'archived', 'all'
    - client: numeric client id (mapped to client_id)
    - q: search in offer_no / client_name
    """
    username_l = (username or "").strip().lower()
    ...
    where = [ _offer_owner_clause().replace("o.", "") ]  # returns clause with o.; remove
    ...
    if client_id is not None:
        # offers table may have client_id (added later); if not present, ignore safely
        try:
