...
def list_offers(user_id: int, username: str, show: str = "active", status: str = "ALL", invoice: str = "ALL", paid: str = "ALL", client: str = "ALL", q: str | None = None):
    """
    show:
    ...
    status_u = (status or "ALL").strip().upper()
    if status_u not in {"ALL", "DRAFT", "SENT", "ACCEPTED"}:
        status_u = "ALL"

    invoice_u = (invoice or "ALL").strip().upper()
    if invoice_u not in {"ALL", "YES", "NO"}:
        invoice_u = "ALL"

    paid_u = (paid or "ALL").strip().upper()
    if paid_u not in {"ALL", "PAID", "UNPAID"}:
        paid_u = "ALL"

    client = (client or "ALL").strip()
    client_clause = "true"
    if client and client != "ALL":
        client_clause = "o.client_name=%s"
    ...
    status_clause = "true"
    if show != "archived" and status_u != "ALL":
        status_clause = "o.status=%s"
        params.append(status_u)


    invoice_clause = "true"
    if invoice_u == "YES":
        invoice_clause = "o.invoice_no is not null"
    elif invoice_u == "NO":
        invoice_clause = "o.invoice_no is null"

    paid_clause = "true"
    if paid_u == "PAID":
        paid_clause = "o.paid=true"
    elif paid_u == "UNPAID":
        paid_clause = "(o.invoice_no is not null and o.paid=false)"

    q_clause = "true"
    if q:
        q_clause = "(coalesce(o.offer_no,'') ilike %s or coalesce(o.client_name,'') ilike %s or coalesce(o.client_email,'') ilike %s)"
        like = f"%{q}%"
    ...
    with get_conn() as conn:
        return conn.execute(
            ...
            where ({_offer_owner_clause()})
              ...
              and {q_clause}
            group by
              o.id, o.offer_no, o.client_name, o.client_email, o.created_at, o.sent_at,
              o.status, o.vat_rate, o.accepted_at, o.archived, o.archived_at,
              o.last_email_to, o.last_email_at, o.last_email_error,
              o.invoice_no, o.invoice_date, o.paid, o.paid_at
