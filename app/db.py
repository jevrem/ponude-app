...
def list_offers(user_id: int, username: str, status: str | None = None, client_id: int | None = None, **kwargs) -> list[dict]:
    ...
    Compatible with older callers passing: show/status/invoice/paid/client/q.
    """
    # Backward-compatible params (older main.py used show/invoice/paid/client/q)
    show = (kwargs.get("show") or "active").strip().lower()
    client = kwargs.get("client")
    q = (kwargs.get("q") or "").strip()

    # normalize status
    status_in = (status or "").strip()
    if not status_in or status_in.upper() == "ALL":
        status_in = ""

    # normalize client_id from "client"
    try:
        if client_id is None and client is not None and str(client).strip() and str(client).upper() != "ALL":
    ...
    # owner clause: prefer user_id match, otherwise match by user_name (case-insensitive)
    where.append("(user_id = %s OR lower(user_name) = %s)")
    params.extend([user_id, username_l])

    # archived/show filter
