...
def build_offer_pdf(output_path: str, offer: Dict[str, Any], client: Dict[str, Any], items: List[Dict[str, Any]]):
    ...
    c.line(40, y, width - 40, y)
    y -= 22
    vat_rate = float(offer.get("vat_rate") or 25)
prices_include_vat = bool(offer.get("prices_include_vat", True))
vr = max(0.0, vat_rate) / 100.0
if prices_include_vat:
    subtotal = total / (1.0 + vr) if vr > 0 else total
    vat_amount = total - subtotal
    c.drawRightString(width - 40, y, f"UKUPNO (bruto): {total:,.2f} €".replace(",", "X").replace(".", ",").replace("X", "."))
    y -= 16
    c.drawRightString(width - 40, y, f"Međuzbroj: {subtotal:,.2f} €".replace(",", "X").replace(".", ",").replace("X", "."))
    y -= 16
    c.drawRightString(width - 40, y, f"PDV {vat_rate:g}%: {vat_amount:,.2f} €".replace(",", "X").replace(".", ",").replace("X", "."))
else:
    vat_amount = total * vr
    total2 = total + vat_amount
    c.drawRightString(width - 40, y, f"UKUPNO (neto): {total:,.2f} €".replace(",", "X").replace(".", ",").replace("X", "."))
    y -= 16
    c.drawRightString(width - 40, y, f"PDV {vat_rate:g}%: {vat_amount:,.2f} €".replace(",", "X").replace(".", ",").replace("X", "."))
    y -= 16
    c.drawRightString(width - 40, y, f"UKUPNO (bruto): {total2:,.2f} €".replace(",", "X").replace(".", ",").replace("X", "."))
