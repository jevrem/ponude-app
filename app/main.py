...
@app.get("/logo.png")
def _logo_png() -> Response:
    """Serve logo without relying on StaticFiles mount (more robust)."""
    path = STATIC_DIR / "logo.png"
    if path.exists():
        return FileResponse(str(path), media_type="image/png")
    # 1x1 transparent png
    return Response(content=base64.b64decode("iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR4nGNgYAAAAAMAASsJTYQAAAAASUVORK5CYII="),
                    media_type="image/png")

@app.get("/static/logo.png")
def _static_logo_png() -> Response:
    """Backward-compatible logo path for older cached templates."""
    return _logo_png()
...
def draw_pdf_background(c, static_dir: str) -> None:
    """Draw a subtle full-page PNG background (watermark) for offer PDFs."""
    ...
    page_w, page_h = A4
    c.saveState()
    # If reportlab supports alpha, keep it subtle
    try:
        c.setFillAlpha(0.10)
    except Exception:
        pass
