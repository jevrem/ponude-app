...
def offer_send(
    ...
    try:
        ...
        # Resolve all IPv4 addresses (A records). Render sometimes has no IPv6 route and/or partial egress.
        infos_587 = socket.getaddrinfo(smtp_host, smtp_port, socket.AF_INET, socket.SOCK_STREAM)
        ips_587 = [info[4][0] for info in infos_587] or []
        # Always also try port 465 as fallback (SMTPS)
        infos_465 = socket.getaddrinfo(smtp_host, 465, socket.AF_INET, socket.SOCK_STREAM)
        ips_465 = [info[4][0] for info in infos_465] or []

        smtp_ssl = (os.getenv("SMTP_SSL") or "0").strip() in {"1", "true", "True", "yes", "YES"}

        last_err: Exception | None = None

        def _try_connect(ip: str, port: int) -> None:
            # Preflight TCP connect so we can distinguish "blocked/timeout" clearly.
            with socket.create_connection((ip, port), timeout=8):
                return

        def _send_starttls(ip: str) -> None:
            with smtplib.SMTP(ip, smtp_port, timeout=15) as s:
                s.ehlo()
                if smtp_tls:
                    s.starttls()
                    s.ehlo()
        ...
        def _send_ssl465(ip: str) -> None:
            with smtplib.SMTP_SSL(ip, 465, timeout=15) as s:
                s.ehlo()
                s.login(smtp_user, smtp_pass)
                s.send_message(msg)
        ...
        if smtp_ssl:
            ...
            # Try 587 on all resolved IPv4s, then fallback to 465 on all IPv4s.
            for ip in ips_587:
                try:
                    _try_connect(ip, smtp_port)
                    _send_starttls(ip)
                    last_err = None
                    break
                except Exception as e:
