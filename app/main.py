...
def offer_send(
    ...
    subj = subject.strip() or f"Ponuda {offer_no} – {client_name}"
    text_body = body.strip() or (
        f"Pozdrav {client_name},

"
        f"U privitku je ponuda {offer_no}.
...
{settings.get('company_name') or username}"
    ...
    html_body = f"""<html>
  <body>
    <p>Pozdrav {html.escape(client_name)},</p>
    <p>U privitku je ponuda <b>{html.escape(str(offer_no))}</b>.</p>
    <p>Lijep pozdrav,<br/>{html.escape(settings.get('company_name') or username)}</p>
  </body>
</html>
"""
...
def offers_page(request: Request):
    ...
    offers = db.list_offers(user_id, username, show=show, status=status, q=q)
    return templates.TemplateResponse(
        "offers.html",
        {
            "request": request,
