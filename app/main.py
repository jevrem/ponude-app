...
def offers_page(request: Request):
    ...
    q = request.query_params.get("q") or ""
        ...
        try:
            client_id_i = int(client) if client else None
        except Exception:
            client_id_i = None

        offers = db.list_offers(user_id, username, show=show, status=status, invoice=invoice, paid=paid, client_id=client_id_i)
