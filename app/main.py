...
def offer_send(
    ...
    try:
        ...
        def _send_via_starttls() -> None:
            with smtplib.SMTP(smtp_ipv4, smtp_port, timeout=15) as s:
                s.ehlo()
                if smtp_tls:
                    s.starttls()
                    s.ehlo()
        ...
        def _send_via_ssl465() -> None:
            with smtplib.SMTP_SSL(smtp_ipv4, 465, timeout=15) as s:
                s.ehlo()
                s.login(smtp_user, smtp_pass)
                s.send_message(msg)
        ...
        if smtp_ssl:
            ...
            try:
                ...
                # Fallback: some hosts/network paths block 587; try SMTPS 465
                _send_via_ssl465()
